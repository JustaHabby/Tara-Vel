 ...
 * - Batch snapshot: New users receive all driver data in a single message
 * 
 * @author TaraVel Team
 */
 ...
 * since the last broadcast, even if the bus hasn't moved. This ensures:
 ...
 * 
 * @example
 * log("User connected", "info");  // Only in dev
...
io.on("connection", (socket) => {
   ...
   * 
   * @emits driversSnapshot - Sent to users with all active driver data
   */
  socket.on("registerRole", safeHandler("registerRole", (role) => {
    ...
    if (role === "user") {
       ...
       * driversSnapshot event with all driver data. This reduces
       * network overhead and improves performance.
       * 
       * OPTIMIZATION: Limit snapshot size to prevent huge payloads
       * when there are many drivers. If MAX_SNAPSHOT_DRIVERS is set,
      ...
      if (MAX_SNAPSHOT_DRIVERS > 0 && driversArray.length > MAX_SNAPSHOT_DRIVERS) {
        ...
        driversArray = driversArray
          ...
          .map(({ lastUpdated, ...driver }) => driver); // Remove lastUpdated - server-only field
   ...
   * 
   * This is the core event handler for location updates. Drivers send this
   ...
   * 
   * The 15-second interval rule ensures that even if a bus is stopped,
   * users still receive regular updates every 15 seconds. This is crucial
  ...
  socket.on("updateLocation", safeHandler("updateLocation", (data) => {
     ...
     * not the last received location. This way, if a bus moves slightly but we don't broadcast
    ...
    drivers[accountId] = {
      ...prevDriver,
      accountId,
      organizationName: organizationName || prevDriver?.organizationName || "No Organization",
      destinationName: destinationName || prevDriver?.destinationName || "Unknown",
    ...
    if (shouldBroadcast) {
      ...
      const reason = !prevDriver ? "initial" : 
                     locationChanged ? "moved" : 
                     passengerDataChanged ? "passengers" : 
                     "interval"; // 15-second heartbeat
   ...
   * the planned route. This is immediately broadcast to all users.
   * 
   * @event routeUpdate
   ...
   * 
   * Called when a client disconnects. Cleans up driver data and mappings.
   */
   ...
   * error Event Handler
   * 
   * Called when a socket error occurs. Logs the error and cleans up.
   */
...
const PORT = 3000;
const HOST = "0.0.0.0"; // Bind to all interfaces (required for Render and other cloud platforms)
